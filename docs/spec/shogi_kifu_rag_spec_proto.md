...
| 言語 | Python 3.12 | CE標準 |
| 棋譜パーサ | python-shogi 1.0.1 | KIF/CSA対応・SFEN変換可能 |
...
【ローカルPC (Windows)】
  KIFファイル
    ↓ python-shogi でパース
    ↓ やねうら王 NNUE で go infinite + stop（3秒）方式で解析
...
- KIFファイルのエンコードはShift-JISとUTF-8が混在する。`chardet`で自動判定し`cp932`に統一する。
...
### 3-5. RAGチェーン（build_combined_context / get_rag_answer）
...
### 3-6. 日本語表記変換（usi_to_japanese / pv_to_japanese）
...
### 3-7. LLM生成（generate_with_fallback）
...
### 4-1. shogi.shogi_silver.positions
...
### 4-2. shogi.shogi_gold.position_features
...
### 4-3. shogi.shogi_gold.game_summary
...
### 4-4. shogi.shogi_gold.floodgate_positions
