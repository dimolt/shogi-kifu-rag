...
def test_gold_table_position_features_schema(spark):
    ...
    for col_name in required_columns:
        assert col_name in column_names, (
            f"Column {col_name} not found in position_features"
        )
...
def test_gold_table_move_quality_logic(spark):
    ...
    if best_moves.count() > 0:
        not_best = best_moves.filter(~col("is_best_move")).count()
    ...
    if blunders.count() > 0:
        not_blunder = blunders.filter(~col("is_blunder")).count()
