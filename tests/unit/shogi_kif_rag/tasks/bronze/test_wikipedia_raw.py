...
from shogi_kif_rag.tasks.bronze import wikipedia_raw
...
def test_fetch_wikipedia_content_正常系のとき_extractを返す(monkeypatch):
    ...
    def fake_get(url, params=None, headers=None, timeout=None):
        ...
        assert headers == wikipedia_raw.WIKIPEDIA_HEADERS
        assert timeout == 10
        return FakeResponse(
            status_code=200,
    ...
    monkeypatch.setattr(wikipedia_raw.requests, "get", fake_get)

    result = wikipedia_raw.fetch_wikipedia_content("矢倉")
...
def test_fetch_wikipedia_content_HTTPステータスが200以外のとき_空文字を返す(monkeypatch):
    ...
    result = wikipedia_raw.fetch_wikipedia_content("存在しない戦法")
