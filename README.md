...
- **使用カタログ**: pytestオプションで自動設定（integration層では `shogi_dev`）
...
- `DATABRICKS_HOST` / `DATABRICKS_CLIENT_ID` / `DATABRICKS_CLIENT_SECRET` が
  環境変数として設定済みであること
- **使用カタログ**: pytestオプションで自動設定（e2e層では `shogi_test`）
...
Job実行が `SUCCESS` 以外で終了した場合、`JobMonitor.wait_for_completion()` が
`JobRunFailedError`を投げ、失敗したTaskの詳細をメッセージに含める。
これによりCIログのみでJob内部の失敗原因まで追跡できる。
...
| 対象 | 既存データに対するSQL/DataFrameロジックの検証 | Job/パイプラインの起動〜完了検証 | 実際のJob起動〜完了までの一気通貫の検証 |
| Job起動 | しない | する | する |
| データ | 既存データを使用 | 既存のdev環境データを使用 | Volumes配置済みデータ + スキーマは毎回クリーン |
| 実行時間 | 秒〜分単位 | 数分〜（Job完了待ち） | 数分〜（Job完了待機、timeout 900秒） |
| 環境 | pytestオプションで自動設定（test: shogi_test / dev: shogi_dev） | pytestオプションで自動設定（shogi_dev） | pytestオプションで自動設定（shogi_test） |

## pytestオプション

テスト実行時にbundle targetとcatalogを柔軟に切り替えるためのpytestコマンドラインオプション：

```bash
# デフォルト値（dev, shogi_dev）
pytest tests/

# 明示的な指定
pytest tests/ --bundle-target=test --catalog=shogi_test
pytest tests/ --bundle-target=prod --catalog=shogi
```

各テスト層のconftest.pyで `pytest_configure()` を通じて自動的に適切な値が設定されます：
- integration層: `--bundle-target=dev --catalog=shogi_dev`
- integration-exec層: `--bundle-target=dev --catalog=shogi_dev`
- e2e層: `--bundle-target=test --catalog=shogi_test`
