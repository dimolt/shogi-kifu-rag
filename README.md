...
- **使用カタログ**: `shogi_dev`（固定、環境変数設定不要）
- **パイプライン/Jobは起動しない**。既存データに対するSQL/DataFrameロジックの検証のみを行う。
  検証対象のテーブル・event_logは、事前に以下いずれかの方法で生成されている必要がある：
...
|---|---|---|---|
| 実行トリガー | 手動 / post-mergeまたはscheduled | 手動 / 定期実行 | CD `deploy-test` ジョブ内（実デプロイ直後） |
| 対象 | 既存データに対するSQL/DataFrameロジックの検証 | Job/パイプラインの起動〜完了検証 | 実際のパイプライン起動〜完了までの一気通貫の検証 |
| パイプライン起動 | しない | する | する |
| データ | 既存のdev環境データを使用 | 既存のdev環境データを使用 | Volumes配置済みデータ + スキーマは毎回クリーン |
| 実行時間 | 秒〜分単位 | 数分〜（Job完了待ち） | 数分〜（パイプライン完了待ち、timeout 900秒） |
| 環境 | devカタログ（テーブルは既存のまま） | devカタログ（テーブルは既存のまま） | testカタログのSilver/Goldスキーマを毎回再作成 |